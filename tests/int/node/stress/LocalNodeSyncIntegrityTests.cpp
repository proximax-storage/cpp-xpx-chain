	...
	namespace {
		...
		void AssertXemBalances(
			...
			for (const auto& expectedBalance : expectedBalances) {
				...
				EXPECT_EQ(expectedBalance.Balance, accountState.Balances.get(Xpx_Id)) << message;
			}
		}
...
#define SINGLE_MULTI_BASED_TEST(TEST_NAME) \
	...
	namespace {
		...
		NamespaceStateHashes RunPruneNamespaceTest(TTestContext& context) {
			...
			auto numAliveBlocks = static_cast<uint32_t>(12 + (utils::TimeSpan::FromHours(1).seconds() / 60) + 10 - 1);
