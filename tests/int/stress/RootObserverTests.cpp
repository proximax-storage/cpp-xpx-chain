...
#define TEST_CLASS RootObserverTests
	...
	namespace {
		...
		public:
			explicit TestContext(uint32_t numAccounts)
					...
					, m_currentCache(m_pPluginManager->createCurrentCache())
					, m_previousCache(m_pPluginManager->createPreviousCache())
				...
				initNemesisCache(m_previousCache, numAccounts);
				initNemesisCache(m_currentCache, numAccounts);
			}
		...
		public:
			...
			void zeroBalances(uint8_t startAccountId, uint8_t numAccounts, Height height) {
				...
				auto accountStateCacheView = m_currentCache.sub<cache::AccountStateCache>().createView();
				for (uint8_t i = startAccountId; i < startAccountId + numAccounts; ++i) {
		...
		public:
			void notify(Height height, NotifyMode mode) {
				...
				executeBlock(m_currentCache, blockElement, pRootObserver, mode, height);
				if (cache::canUpdatePreviousCache(height, Height(Effective_Balance_Range)) ) {
					pBlock = createBlock(height - Height(Effective_Balance_Range));
					auto oldBLockElement = test::BlockToBlockElement(*pBlock);
					executeBlock(m_previousCache, oldBLockElement, pRootObserver, mode, height);
				}
			}
		...
		public:
			...
			void assertSingleEffectiveBalance(
				...
				auto currentCacheView = m_currentCache.createView();
				auto previousCacheView = m_previousCache.createView();
				...
				EXPECT_EQ(expectedBalance, balanceView.getEffectiveBalance(Key{ { accountId } }, height)) << message;
			}
	...
	TEST(TEST_CLASS, ExecuteCalculatesImportancesCorrectly_WithDifferentBalances) {
		// Arrange: create a context with 10 accounts
		TestContext context(10);
		context.addAccounts(1, 10, Height(2));
		context.notify(Height(2), NotifyMode::Commit);
