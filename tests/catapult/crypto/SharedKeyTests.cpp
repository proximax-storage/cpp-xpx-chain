...
#define TEST_CLASS SharedKeyTests
	...
	namespace {
		...
		void Hkdf_Hmac_Sha256(const RawBuffer& sharedSecret, const RawBuffer& salt, const RawBuffer& label, std::vector<uint8_t>& output) {
			...
			buffer.resize(Hash256_Size + label.Size + sizeof(uint8_t));

			size_t repetitions = (output.size() + Hash256_Size - 1) / Hash256_Size;
			size_t position = 0;
			...
			auto numBytesWritten = std::min(output.size() - position, Hash256_Size);
			std::memcpy(&output[position], previousOkm.data(), numBytesWritten);
			position += numBytesWritten;
			++counter;

			for (; counter <= repetitions; ++counter) {
				std::memcpy(buffer.data(), previousOkm.data(), Hash256_Size);
				utils::memcpy_cond(buffer.data() + Hash256_Size, label.pData, label.Size);
				std::memcpy(buffer.data() + Hash256_Size + label.Size, &counter, sizeof(uint8_t));

				Hmac_Sha256(prk, buffer, previousOkm);

				numBytesWritten = std::min(output.size() - position, Hash256_Size);
				std::memcpy(&output[position], previousOkm.data(), numBytesWritten);
				position += numBytesWritten;
			}
		}
	...
	TEST(TEST_CLASS, Hkdf_Hmac_Sha256_Test_Vector_1) {
		// Arrange:
		auto salt = test::ToVector("000102030405060708090A0B0C");
		auto sharedSecret = test::ToVector("0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B");
		auto label = test::ToVector("F0F1F2F3F4F5F6F7F8F9");
		auto expected = test::ToVector("3CB25F25FAACD57A90434F64D0362F2A2D2D0A90CF1A5A4C5DB02D56ECC4C5BF34007208D5B887185865");
	...
	TEST(TEST_CLASS, Hkdf_Hmac_Sha256_Test_Vector_2) {
		// Arrange:
		auto salt = test::ToVector(
				"606162636465666768696A6B6C6D6E6F707172737475767778797A7B7C7D7E7F"
				"808182838485868788898A8B8C8D8E8F909192939495969798999A9B9C9D9E9F"
				"A0A1A2A3A4A5A6A7A8A9AAABACADAEAF");
		auto sharedSecret = test::ToVector(
				"000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F"
				"202122232425262728292A2B2C2D2E2F303132333435363738393A3B3C3D3E3F"
				"404142434445464748494A4B4C4D4E4F");
		auto label = test::ToVector(
				"B0B1B2B3B4B5B6B7B8B9BABBBCBDBEBFC0C1C2C3C4C5C6C7C8C9CACBCCCDCECF"
				"D0D1D2D3D4D5D6D7D8D9DADBDCDDDEDFE0E1E2E3E4E5E6E7E8E9EAEBECEDEEEF"
				"F0F1F2F3F4F5F6F7F8F9FAFBFCFDFEFF");
		auto expected = test::ToVector(
	...
	TEST(TEST_CLASS, Hkdf_Hmac_Sha256_Test_Vector_3) {
		...
		auto sharedSecret = test::ToVector("0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B");
		auto label = test::ToVector("");
		auto expected = test::ToVector("8DA4E775A563C18F715F802A063C5A31B8A11F5C5EE1879EC3454E5F3C738D2D9D201395FAA4B61A96C8");
	...
	TEST(TEST_CLASS, Hkdf_Hmac_Sha256_32_Tests) {
		...
		auto label = test::ToVector("6361746170756c74");
	...
	namespace {
		...
		auto CreateKeyPair(const Key& privateKey, DerivationScheme derivationScheme) {
			PrivateKey key;
			std::memcpy(const_cast<unsigned char *>(key.data()), privateKey.data(), privateKey.Size);
			return KeyPair::FromPrivate(std::move(key), derivationScheme);
		}
		...
		void AssertDerivedSharedResult(
			...
			auto otherPublicKey1 = test::GenerateKeyPair(DerivationScheme::Ed25519_Sha3).publicKey();
			...
			auto keyPair1 = CreateKeyPair(privateKey1, DerivationScheme::Ed25519_Sha2);
			auto keyPair2 = CreateKeyPair(privateKey2, DerivationScheme::Ed25519_Sha2);
...
#define SHARED_BASED_TEST(TEST_NAME) \
	...
	SHARED_BASED_TEST(MutualSharedResultsAreEqualFixedKeys) {
		// Arrange:
		...
		auto keyPair1 = KeyPair::FromPrivate(std::move(privateKey1), Vrf_Key_Derivation_Scheme);
		auto keyPair2 = KeyPair::FromPrivate(std::move(privateKey2), Vrf_Key_Derivation_Scheme);

		// Act:
		auto sharedResult1 = TTraits::DeriveSharedResult(keyPair1, keyPair2.publicKey());
		auto sharedResult2 = TTraits::DeriveSharedResult(keyPair2, keyPair1.publicKey());
		CATAPULT_LOG(info) << "Shared#0: " <<sharedResult1 << "\nShared#1: " << sharedResult2;
		// Assert:
		EXPECT_EQ(sharedResult2, sharedResult1);
	}
