...
#define TEST_CLASS KeyLinkValidatorTests
	...
	namespace {
		...
		void AssertValidation(
			...
			auto cache = test::CoreSystemCacheFactory::Create(config.ToConst());
			{
	...
	TEST(TEST_CLASS, CanLinkWhenLinkIsUnset) {
		AssertValidation(validators::ValidationResult::Success, Key(), test::GenerateRandomByteArray<Key>(), model::AccountLinkAction::Link);
	}
	...
	TEST(TEST_CLASS, CanUnlinkWhenLinkIsSetAndLinkedPublicKeyMatches) {
		...
		AssertValidation(validators::ValidationResult::Success, linkedPublicKey, linkedPublicKey, model::AccountLinkAction::Unlink);
	}
	...
	TEST(TEST_CLASS, CannotUnlinkWhenLinkIsSetAndLinkedPublicKeyDoesNotMatch) {
		AssertValidation(
				...
				test::GenerateRandomByteArray<Key>(),
				test::GenerateRandomByteArray<Key>(),
				model::AccountLinkAction::Unlink);
	}

	TEST(TEST_CLASS, CannotUnlinkWhenUnset) {
		AssertValidation(Accessor::Failure_Inconsistent_Unlink_Data, Key(), Key(), model::AccountLinkAction::Unlink);
	}
