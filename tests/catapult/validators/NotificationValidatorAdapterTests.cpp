...
#define TEST_CLASS NotificationValidatorAdapterTests
	...
	TEST(TEST_CLASS, ExtractsAndForwardsNotificationsFromEntity) {
		...
		RunTest(ValidationResult::Success, [](const auto& adapter, const auto& validator) {
			...
			ASSERT_EQ(7u + 4u, validator.notificationTypes().size());
			...
			EXPECT_EQ(model::Core_Signer_v1_Notification, validator.notificationTypes()[3]);
			EXPECT_EQ(model::Core_Transaction_Fee_v1_Notification, validator.notificationTypes()[4]);
			EXPECT_EQ(model::Core_Balance_Debit_v1_Notification, validator.notificationTypes()[5]);
			EXPECT_EQ(model::Core_Signature_v2_Notification, validator.notificationTypes()[6]);

			// - mock transaction notifications
			EXPECT_EQ(mocks::Mock_Validator_1_Notification, validator.notificationTypes()[7]);
			EXPECT_EQ(mocks::Mock_All_1_Notification, validator.notificationTypes()[8]);
			EXPECT_EQ(mocks::Mock_Validator_2_Notification, validator.notificationTypes()[9]);
			EXPECT_EQ(mocks::Mock_All_2_Notification, validator.notificationTypes()[10]);
	...
	TEST(TEST_CLASS, DelegatesWhenTypeMatches_Success) {
		...
		AssertMockTransactionValidation(ValidationResult::Success, 11);
	}

	TEST(TEST_CLASS, DelegatesWhenTypeMatches_Neutral) {
		// Assert: all notifications should be processed
		AssertMockTransactionValidation(ValidationResult::Neutral, 11);
	}
