...
#define TEST_CLASS BlockChainProcessorTests
	...
	namespace {
		...
		public:
			ProcessorTestContext() {
				Handlers.BatchEntityProcessor = [this](const auto& height, const auto& timestamp, const auto& entities, const auto& state) {
		...
		public:
			void assertBlockHitPredicateCalls(const BlockElements& elements) {
				...
				for (auto params : allParams) {
					...
					Amount expectedEffectiveBalance{0};
			...
			void assertBatchEntityProcessorCalls(SyncState& state, const BlockElements& elements) {
				...
				for (auto params : allParams) {
					...
					EXPECT_EQ(&state.observerState().Cache, &params.State.Cache) << message;
					++i;
				}
			}
	...
	namespace {
		void AssertCanProcessValidElements(BlockElements& elements, size_t numExpectedBlocks) {
			...
			context.assertBlockHitPredicateCalls(elements);
