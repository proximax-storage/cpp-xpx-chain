...
#define TEST_CLASS NodeUtilsTests
	...
	TEST(TEST_CLASS, SeedNodeContainerAddsOnlyLocalNodeWhenThereAreNoStaticNodes) {
		...
		auto expectedContents = test::BasicNodeDataContainer{
			{ crypto::KeyPair::FromString(hexPrivateKey, Node_Boot_Key_Derivation_Scheme).publicKey(), "LOCAL", ionet::NodeSource::Local }
		};
	...
	TEST(TEST_CLASS, SeedNodeContainerSucceedsWhenMaxStringLengthsAreUsed) {
		...
		auto expectedContents = test::BasicNodeDataContainer{
			{ crypto::KeyPair::FromString(hexPrivateKey, Node_Boot_Key_Derivation_Scheme).publicKey(), std::string(255, 'l'), ionet::NodeSource::Local },
