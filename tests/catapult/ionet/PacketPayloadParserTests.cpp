	...
	namespace {
		...
		struct ContainsSingleEntityTraits {
			...
			static void PrepareBufferWithOverflowSize(ByteBuffer& buffer, uint32_t size) {
				...
				buffer.resize(std::max<uint32_t>(sizeof(model::BlockHeaderV4), size));
		...
		struct ExtractEntityOffsetsMultiEntityTraits : ExtractEntityOffsetsTraits {
			static void PrepareBufferWithOverflowSize(ByteBuffer& buffer, uint32_t size) {
				...
				constexpr auto Base_Buffer_Size = Num_Full_Blocks * sizeof(model::BlockHeaderV4);
				buffer.resize(Base_Buffer_Size + std::max<uint32_t>(sizeof(model::BlockHeaderV4), size));
				...
				for (auto i = 0u; i <= Num_Full_Blocks; ++i) {
					...
					test::SetBlockAt(buffer, i * sizeof(model::BlockHeaderV4), blockSize);
				}
			}
		};
...
#define BUFFER_SINGLE_ENTITY_TEST(TEST_NAME) \
	...
	BUFFER_FAILURE_TEST(CannotExtractFromBufferWhenBufferExpandsBeyondLastEntity) {
		...
		ByteBuffer buffer;
		TTraits::PrepareBufferWithOverflowSize(buffer, sizeof(model::BlockHeaderV4));
	...
	BUFFER_SINGLE_ENTITY_TEST(CanExtractSingleBlockWithoutTransactions) {
		...
		ByteBuffer buffer(sizeof(model::BlockHeaderV4));
		test::SetBlockAt(buffer, 0);
	...
	TEST(TEST_CLASS, CanExtractMultipleBlocks_ExtractEntityOffsets) {
		...
		EXPECT_EQ(std::vector<size_t>({ 0, sizeof(model::BlockHeaderV4), sizeof(model::BlockHeaderV4) + Block_Transaction_Size }), offsets);
	}
