...
namespace catapult { namespace cache {
	...
	public:
		...
		explicit BalanceView(
				ReadOnlyAccountStateCache currentCache,
				ReadOnlyAccountStateCache previousCache,
				const Height& effectiveBalanceHeight)
				: m_currentCache(currentCache)
				, m_previousCache(previousCache)
	...
	public:
		/// Gets the effective balance for \a publicKey at \a height
		Amount getEffectiveBalance(const Key& publicKey, Height height) const;

		/// Gets the effective balance for \a publicKey at \a height
		Amount getBalance(const Key& publicKey) const;
	...
	private:
		ReadOnlyAccountStateCache m_currentCache;
		ReadOnlyAccountStateCache m_previousCache;
