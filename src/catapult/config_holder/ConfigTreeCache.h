...
namespace catapult { namespace config {
	...
	public:
		...
		Height GetLowerOrEqualHeightConfig(const Height& height){
			auto iterConfig = getLowerOrEqual<const std::map<Height, ConfigTreeCache::ConfigRoot>::const_iterator,
					std::map<Height, ConfigTreeCache::ConfigRoot>>(height, m_configs);

			if( iterConfig != m_configs.end())
				return iterConfig->first;
	...
	private:
		...
		template<typename ReturnIteratorType, typename MapType>
		ReturnIteratorType getLowerOrEqual(const Height& height, const MapType& map)
		{
			auto iter = map.lower_bound(height);

			if (iter != map.end() && iter->first != height) {
				iter = (iter == map.begin()) ? map.end() : --iter;
			} else if (iter == map.end()) {
				if (!map.empty()) {
					--iter;
				}
			}
