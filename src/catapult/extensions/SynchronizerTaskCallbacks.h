...
namespace catapult { namespace extensions {
	...
	/// \a pPacketIoPicker is used to select peers and \a remoteApiFactory wraps an api around peers.
	...
	thread::TaskCallback CreateSynchronizerTaskCallback(
		...
		chain::RemoteApiForwarder forwarder(pPacketIoPicker, state.pluginManager().transactionRegistry(), syncTimeout, taskName);
	...
	thread::TaskCallback CreateChainSyncAwareSynchronizerTaskCallback(
		...
		auto synchronize = CreateSynchronizerTaskCallback(std::move(synchronizer), remoteApiFactory, pPacketIoPicker, state, taskName);
