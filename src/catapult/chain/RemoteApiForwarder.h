...
namespace catapult { namespace chain {
	...
	public:
		/// Creates a forwarder around a peer selector (\a pPacketIoPicker) with a connection \a timeout
		/// or a ready packet io pair (\a pPacketIoPair) given a transaction registry (\a transactionRegistry)
		/// and a friendly name (\a operationName).
		RemoteApiForwarder(
				std::weak_ptr<net::PacketIoPicker> pPacketIoPicker,
				const model::TransactionRegistry& transactionRegistry,
				const utils::TimeSpan& timeout,
				const std::string& operationName,
				std::shared_ptr<ionet::NodePacketIoPair> pPacketIoPair = nullptr)
				: m_pPacketIoPicker(pPacketIoPicker)
				, m_transactionRegistry(transactionRegistry)
				, m_timeout(timeout)
				, m_operationName(operationName)
				, m_pPacketIoPair(pPacketIoPair)
		{}
	...
	public:
		...
		thread::future<ionet::NodeInteractionResult> processSync(TRemoteApiAction action, TRemoteApiFactory apiFactory) const {
			auto pPacketIoPicker = m_pPacketIoPicker.lock();
			if (pPacketIoPicker) {
				return processSync(action, apiFactory, pPacketIoPicker->pickOne(m_timeout));
			} else if (m_pPacketIoPair) {
				return processSync(action, apiFactory, *m_pPacketIoPair);
			} else {
				return processSync(action, apiFactory, ionet::NodePacketIoPair{});
			}
		}

	private:
		template<typename TRemoteApiAction, typename TRemoteApiFactory>
		thread::future<ionet::NodeInteractionResult> processSync(
				TRemoteApiAction action, TRemoteApiFactory apiFactory, const ionet::NodePacketIoPair& packetIoPair) const {
			if (!packetIoPair) {
	...
	private:
		std::weak_ptr<net::PacketIoPicker> m_pPacketIoPicker;
		const model::TransactionRegistry& m_transactionRegistry;
		utils::TimeSpan m_timeout;
		std::string m_operationName;
		std::shared_ptr<ionet::NodePacketIoPair> m_pPacketIoPair;
	};
}}
