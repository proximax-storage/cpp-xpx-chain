...
namespace catapult { namespace tools {
	...
	public:
		...
		int run(const Options&) override final {
			...
			auto p2pNodes = LoadPeers(m_resourcesPath, config.Network.Info.Identifier);
			auto apiNodes = LoadOptionalApiPeers(m_resourcesPath, config.Network.Info.Identifier);
