...
namespace statichash{
	...
	template<>
	inline constexpr unsigned char get_byte(const char * input, int i) {
		return input[i];
	}

	template<>
	inline constexpr unsigned char get_byte(char * input, int i) {
		return input[i];
	}

	template<>
	inline unsigned char get_byte(std::string input, int i) {
		return input[i];
	}


	template<typename T>
	inline constexpr size_t get_size(T input) {
		return sizeof(T);
	}

	template<>
	inline constexpr size_t get_size(const char * input) {
		return get_string_size(input);
	}

	template<>
	inline constexpr size_t get_size(char * input) {
		return get_string_size(input);
	}

	template<>
	inline size_t get_size(std::string input) {
		return input.size();
	}
