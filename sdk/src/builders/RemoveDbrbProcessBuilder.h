...
#include "plugins/txes/dbrb/src/model/RemoveDbrbProcessTransaction.h"
#include <vector>

namespace catapult { namespace builders {

	/// Builder for a catapult upgrade transaction.
	class RemoveDbrbProcessBuilder : public TransactionBuilder {
	public:
		using Transaction = model::RemoveDbrbProcessTransaction;
		using EmbeddedTransaction = model::EmbeddedRemoveDbrbProcessTransaction;

		/// Creates a catapult upgrade builder for building a catapult upgrade transaction from \a signer
		/// for the network specified by \a networkIdentifier.
		RemoveDbrbProcessBuilder(model::NetworkIdentifier networkIdentifier, const Key& signer);
