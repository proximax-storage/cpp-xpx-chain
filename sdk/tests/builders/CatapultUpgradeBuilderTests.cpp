...
#include "src/builders/CatapultUpgradeBuilder.h"
#include "tests/builders/test/BuilderTestUtils.h"
#include "tests/test/nodeps/Filesystem.h"

namespace catapult { namespace builders {

#define TEST_CLASS CatapultUpgradeBuilderTests

	namespace {
		using RegularTraits = test::RegularTransactionTraits<model::CatapultUpgradeTransaction>;
		using EmbeddedTraits = test::EmbeddedTransactionTraits<model::EmbeddedCatapultUpgradeTransaction>;
		...
		void AssertTransactionProperties(const TransactionProperties& expectedProperties, const TTransaction& transaction) {
			...
			EXPECT_EQ(expectedProperties.NewCatapultVersion, transaction.NewCatapultVersion);
		}

		template<typename TTraits>
		void AssertCanBuildTransaction(
			const TransactionProperties& expectedProperties,
			const consumer<CatapultUpgradeBuilder&>& buildTransaction) {
			// Arrange:
			auto networkId = static_cast<model::NetworkIdentifier>(0x62);
			auto signer = test::GenerateRandomByteArray<Key>();

			// Act:
			CatapultUpgradeBuilder builder(networkId, signer);
			...
			EXPECT_EQ(model::Entity_Type_Catapult_Upgrade, pTransaction->Type);
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TRAITS_BASED_TEST(CanSetNewCatapultVersion) {
		// Arrange:
		auto expectedProperties = TransactionProperties();
		expectedProperties.NewCatapultVersion = CatapultVersion{7};

		// Assert:
		AssertCanBuildTransaction<TTraits>(expectedProperties, [](auto& builder) {
			builder.setNewCatapultVersion(CatapultVersion{7});
		});
	}
