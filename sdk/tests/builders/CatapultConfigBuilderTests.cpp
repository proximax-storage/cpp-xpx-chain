...
#include "src/builders/CatapultConfigBuilder.h"
#include "tests/builders/test/BuilderTestUtils.h"
#include "tests/test/nodeps/Filesystem.h"

namespace catapult { namespace builders {

#define TEST_CLASS CatapultConfigBuilderTests

	namespace {
		using RegularTraits = test::RegularTransactionTraits<model::CatapultConfigTransaction>;
		using EmbeddedTraits = test::EmbeddedTransactionTraits<model::EmbeddedCatapultConfigTransaction>;
		...
		void AssertCanBuildTransaction(
			...
			const consumer<CatapultConfigBuilder&>& buildTransaction) {
			// Arrange:
			auto networkId = static_cast<model::NetworkIdentifier>(0x62);
			auto signer = test::GenerateRandomByteArray<Key>();

			// Act:
			CatapultConfigBuilder builder(networkId, signer);
			...
			EXPECT_EQ(model::Entity_Type_Catapult_Config, pTransaction->Type);
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TRAITS_BASED_TEST(CanSetRawBlockChainConfig) {
		...
		// Assert:
		AssertCanBuildTransaction<TTraits>(expectedProperties, [](auto& builder) {
			std::string blockChainConfig("aaa");
			builder.setBlockChainConfig(RawBuffer{ reinterpret_cast<uint8_t*>(blockChainConfig.data()), blockChainConfig.size() });
		});
	}
