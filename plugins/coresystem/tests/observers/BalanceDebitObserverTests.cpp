...
#define DEFINE_BALANCE_OBSERVATION_TESTS(TEST_NAME) \
	...
	namespace {
		...
		struct SingleMosaicTraits {
			static auto CreateNotification(const Key& sender) {
				return model::BalanceDebitNotification(sender, test::UnresolveXor(Currency_Mosaic_Id), Amount(234), 0);
			}
	...
	namespace {
		struct MultipleMosaicTraits {
			static auto CreateNotification(const Key& sender) {
				return model::BalanceDebitNotification(sender, test::UnresolveXor(MosaicId(12)), Amount(234), 0);
			}
