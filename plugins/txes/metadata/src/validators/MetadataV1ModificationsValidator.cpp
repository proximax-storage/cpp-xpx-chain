...
namespace catapult { namespace validators {
	...
	namespace {
		ValidationResult validate(const Notification& notification, const ValidatorContext& context, const uint8_t& maxFields) {
			...
			for (const auto& modification : notification.Modifications) {
				...
				if (modification->ModificationType == model::MetadataV1ModificationType::Add && removed.count(key) == 0) {
					auto value = hasher(std::string(modification->ValuePtr(), modification->ValueSize));
					added.emplace(key, value);
				} else if (modification->ModificationType == model::MetadataV1ModificationType::Del && added.count(key) == 0) {
					removed.emplace(key);
				}
			}
	...
	DECLARE_STATEFUL_VALIDATOR(MetadataV1Modifications, Notification)() {
		return MAKE_STATEFUL_VALIDATOR(MetadataV1Modifications, [](const Notification& notification, const ValidatorContext& context) {
