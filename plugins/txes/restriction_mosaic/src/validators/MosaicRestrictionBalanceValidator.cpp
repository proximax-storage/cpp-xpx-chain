...
namespace catapult { namespace validators {
	...
	namespace {
		...
		ValidationResult CheckTransferAuthorization(
				...
				const ValidatorContext& context) {
			return ProcessMosaicRules(notification.MosaicId, context, [&notification, &resolvers = context.Resolvers, identifier = context.NetworkIdentifier](
					const auto& cache,
					const auto& mosaicRules) {
				auto isSenderAuthorized = cache::EvaluateMosaicRestrictionResolvedRulesForAddress(cache, model::PublicKeyToAddress(notification.Sender, identifier), mosaicRules);
		...
		ValidationResult CheckDebitAuthorization(const model::BalanceDebitNotification<1>& notification, const ValidatorContext& context) {
			return ProcessMosaicRules(notification.MosaicId, context, [&notification, identifier = context.NetworkIdentifier](const auto& cache, const auto& mosaicRules) {
				return cache::EvaluateMosaicRestrictionResolvedRulesForAddress(cache, model::PublicKeyToAddress(notification.Sender, identifier), mosaicRules);
			});
		}
	}

	DEFINE_STATEFUL_VALIDATOR_WITH_TYPE(MosaicRestrictionBalanceTransfer, model::BalanceTransferNotification<1>, CheckTransferAuthorization)
	DEFINE_STATEFUL_VALIDATOR_WITH_TYPE(MosaicRestrictionBalanceDebit, model::BalanceDebitNotification<1>, CheckDebitAuthorization)
}}
