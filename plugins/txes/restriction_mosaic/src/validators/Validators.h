...
namespace catapult { namespace validators {
	...
	DECLARE_STATEFUL_VALIDATOR(MosaicRestrictionBalanceTransfer, model::BalanceTransferNotification<1>)();

	/// Validator that applies to balance debit notifications and validates that:
	/// - recipient is authorized to receive mosaic
	DECLARE_STATEFUL_VALIDATOR(MosaicRestrictionBalanceDebit, model::BalanceDebitNotification<1>)();
}}
