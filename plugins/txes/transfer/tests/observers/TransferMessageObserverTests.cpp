...
#define TEST_CLASS TransferMessageObserverTests
	...
	DEFINE_COMMON_OBSERVER_TESTS(TransferMessageV2, 0, Address(), config::CatapultDirectory(""))
...
#define MESSAGE_OBSERVER_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		class TransferMessageV2ObserverTestContext {
		public:
			explicit TransferMessageV2ObserverTestContext(NotifyMode notifyMode, Height height = Height(444))
		...
		public:
			void observe(const Address& recipient, const model::TransferMessageNotification<2>& notification) {
				auto pObserver = CreateTransferMessageV2Observer(0x1122334455667788, recipient, config::CatapultDirectory(m_tempDir.name()));
	...
	namespace {
		...
		void AssertInvalidMarker(const std::vector<uint8_t>& message) {
			// Arrange:
			TransferMessageV2ObserverTestContext context(TTraits::Notify_Mode);
			...
			auto notification = model::TransferMessageNotification<2>(sender, unresolvedRecipient, messageSize, message.data());
	...
	MESSAGE_OBSERVER_TRAITS_BASED_TEST(MessageIsWrittenWhenMarkerAndRecipientBothMatch) {
		// Arrange:
		TransferMessageV2ObserverTestContext context(TTraits::Notify_Mode, Height(789));
