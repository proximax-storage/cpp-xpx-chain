...
namespace catapult { namespace model {
	...
	using TransactionType = EndOperationTransaction;

#define TEST_CLASS EndOperationTransactionTests
	...
	namespace {
		...
		void AssertTransactionHasExpectedProperties() {
			// Assert:
			EXPECT_EQ(Entity_Type_EndOperation, T::Entity_Type);
			EXPECT_EQ(1u, T::Current_Version);
		}
	}

	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS(EndOperation)
	...
	namespace {
		struct EndOperationTransactionTraits {
	...
	DEFINE_ATTACHMENT_POINTER_TESTS(TEST_CLASS, EndOperationTransactionTraits)
