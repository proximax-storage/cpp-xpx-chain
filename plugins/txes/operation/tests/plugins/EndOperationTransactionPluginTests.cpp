...
#define TEST_CLASS EndOperationTransactionPluginTests

	// region TransactionPlugin

	namespace {
		DEFINE_TRANSACTION_PLUGIN_TEST_TRAITS(EndOperation, 1, 1,)
		...
		auto CreateTransaction() {
			return test::CreateEndOperationTransaction<typename TTraits::TransactionType>(Num_Mosaics);
		}
	}

	DEFINE_BASIC_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS_ONLY_EMBEDDABLE(TEST_CLASS,,, Entity_Type_EndOperation)
	...
	PLUGIN_TEST(CanPublishCorrectNumberOfNotifications) {
		...
		// Assert:
		ASSERT_EQ(2u + Num_Mosaics, sub.numNotifications());
		auto i = 0u;
