...
namespace catapult { namespace validators {
	...
	DEFINE_STATELESS_VALIDATOR(PluginConfig, [](const auto& notification) {
		if (notification.Name == "catapult.plugins.contract") {
			try {
				(void)config::ContractConfiguration::LoadFromBag(notification.Bag);
			} catch (...) {
				return Failure_Contract_Plugin_Config_Malformed;
			}
		}
