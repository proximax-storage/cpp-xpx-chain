...
namespace catapult { namespace model {
	...
	using TransactionType = ModifyMultisigAccountAndReputationTransaction;

#define TEST_CLASS ModifyMultisigAccountAndReputationTransactionTests
	...
	namespace {
		...
		void AssertTransactionHasExpectedProperties() {
			// Assert:
			EXPECT_EQ(Entity_Type_Modify_Multisig_Account_And_Reputation, static_cast<EntityType>(T::Entity_Type));
			EXPECT_EQ(3u, static_cast<uint8_t>(T::Current_Version));
		}
	}

	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS(ModifyMultisigAccountAndReputation)
	...
	namespace {
		struct ModifyMultisigAccountAndReputationTransactionTraits {
	...
	DEFINE_ATTACHMENT_POINTER_TESTS(TEST_CLASS, ModifyMultisigAccountAndReputationTransactionTraits)
