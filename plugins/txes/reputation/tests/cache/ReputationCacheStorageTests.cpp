...
#define TEST_CLASS ReputationCacheStorageTests
	...
	TEST(TEST_CLASS, CanSaveSingleEntry) {
		...
		// Act:
		ReputationCacheStorage::Save(std::make_pair(entry.key(), entry), context.outputStream());
	...
	TEST(TEST_CLASS, CanSaveMultipleEntries) {
		...
		// Act:
		ReputationCacheStorage::Save(std::make_pair(entry1.key(), entry1), context.outputStream());
		ReputationCacheStorage::Save(std::make_pair(entry2.key(), entry2), context.outputStream());
	...
	namespace {
		...
		struct ReputationCacheStorageTraits {
			using KeyType = Key;
			using ValueType = state::ReputationEntry;

			using StorageType = ReputationCacheStorage;
		...
		using LookupCacheStorageTests = test::LookupCacheStorageTests<ReputationCacheStorageTraits>;
...
#define LOAD_TEST(TEST_NAME) \
	...
	LOAD_TEST(CanLoadSingleEntry) {
		AssertCanLoadSingleEntry<TTraits>();
	}
