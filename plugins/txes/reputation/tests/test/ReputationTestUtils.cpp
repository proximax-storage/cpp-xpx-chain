...
namespace catapult { namespace test {
	...
	std::unique_ptr<model::EmbeddedModifyMultisigAccountAndReputationTransaction> CreateModifyMultisigAccountAndReputationTransaction(
		...
		using TransactionType = model::EmbeddedModifyMultisigAccountAndReputationTransaction;
		...
		pTransaction->Type = model::Entity_Type_Modify_Multisig_Account_And_Reputation;
	...
	namespace {
		state::ReputationEntry& GetOrCreateEntry(cache::ReputationCacheDelta& reputationCache, const Key& key) {
			...
			return reputationCache.get(key);
		}
	}
