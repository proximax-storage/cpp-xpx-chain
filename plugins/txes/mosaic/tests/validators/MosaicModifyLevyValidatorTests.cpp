...
#define TEST_CLASS MosaicModifyLevyValidatorTests
		...
		namespace {
			...
			model::MosaicLevyRaw CreateValidMosaicLevyRaw(model::LevyType type = model::LevyType::Absolute,
					UnresolvedAddress recipient = test::GenerateRandomByteArray<UnresolvedAddress>(),
				    UnresolvedMosaicId mosaicId = UnresolvedMosaicId(1000), Amount fee = Amount(100)) {
			...
			void AddMosaic(cache::CatapultCacheDelta& cache, MosaicId id, Height height, Amount supply, const Key& owner, model::MosaicFlags flags) {
			...
			void AssertLevyParameterTest(ValidationResult expectedResult,  const model::MosaicLevyRaw& levy, bool validRecipient,
				const MosaicId& recipientBalanceMosaicId, bool validSigner, LevySetupFunc action) {
				...
				auto xORLevy = model::MosaicLevyRaw(levy.Type, levy.Recipient, test::UnresolveXor(MosaicId(levy.MosaicId.unwrap())),levy.Fee);
				auto notification =  model::MosaicModifyLevyNotification<1>(test::UnresolveXor(Currency_Mosaic_Id), xORLevy, signer);
		...
		TEST(TEST_CLASS, NonTransferableLevy) {
			auto recipient = test::GenerateRandomByteArray<Address>();
			AssertLevyParameterTest(Failure_Mosaic_Non_Transferable,
				CreateValidMosaicLevyRaw(
