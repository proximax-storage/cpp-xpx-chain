...
namespace catapult { namespace observers {
	...
	namespace {
		...
		auto ApplyNotification(
			...
			return state::MosaicEntry(notification.MosaicId, definition, notification.Levy);
		}
	}
	...
	DEFINE_OBSERVER(MosaicDefinition, model::MosaicDefinitionNotification<1>, [](const auto& notification, const ObserverContext& context) {
		...
		if (mosaicIter.tryGet()) {
			...
			cache.insert(state::MosaicEntry(notification.MosaicId, definition, notification.Levy));
		}
	});
}}
