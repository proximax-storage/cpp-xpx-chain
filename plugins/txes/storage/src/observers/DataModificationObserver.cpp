...
namespace catapult { namespace observers {
	...
	DECLARE_OBSERVER(DataModification, Notification)(const std::shared_ptr<DriveQueue>& pDriveQueue) {
		return MAKE_OBSERVER(DataModification, Notification, ([pDriveQueue](const Notification& notification, const ObserverContext& context) {
			...
			utils::OffboardReplicatorsFromDrive(notification.DriveKey, offboardingReplicators, context, rng);
		  	utils::PopulateDriveWithReplicators(notification.DriveKey, pDriveQueue, context, rng);
		}))
	}
}}
