...
namespace catapult::observers {
	...
	void StorageExternalManagementObserverImpl::updateStorageState(
	...
	void StorageExternalManagementObserverImpl::addToConfirmedStorage(
			const ObserverContext& context,
			const Key& driveKey,
			const std::set<Key>& replicators) const {
		auto& driveCache = context.Cache.sub<cache::BcDriveCache>();
		auto driveIter = driveCache.find(driveKey);
		auto& driveEntry = driveIter.get();
	...
	void StorageExternalManagementObserverImpl::allowOwnerManagement(
