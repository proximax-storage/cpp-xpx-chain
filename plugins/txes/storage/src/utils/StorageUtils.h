...
namespace catapult { namespace utils {
	...
	// TODO: Check changes (DownloadAction -> DownloadTransaction, pFile->FileSize -> pFile->DownloadSize.unwrap())
    inline Amount CalculateFileDownload(const model::DownloadTransaction* pFile, uint16_t fileCount) {
		uint64_t totalSize = 0u;
		for (auto i = 0u; i < fileCount; ++i, ++pFile) {
			totalSize += pFile->DownloadSize.unwrap();
		}
