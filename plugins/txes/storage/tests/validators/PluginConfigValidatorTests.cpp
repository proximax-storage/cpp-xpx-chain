...
#define TEST_CLASS PluginConfigValidatorTests
	...
	DEFINE_COMMON_VALIDATOR_TESTS(StoragePluginConfig,)

	namespace {
		struct PluginConfigTraits {
			static auto CreatePluginConfigValidator() {
				return validators::CreateStoragePluginConfigValidator();
			}
	...
	DEFINE_PLUGIN_CONFIG_VALIDATOR_TESTS(TEST_CLASS, PluginConfigTraits, 1, storage, Storage)
}}
