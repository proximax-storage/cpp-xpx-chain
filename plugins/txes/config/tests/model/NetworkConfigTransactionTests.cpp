...
namespace catapult { namespace model {
	...
	using TransactionType = NetworkConfigTransaction;

#define TEST_CLASS NetworkConfigTransactionTests
	...
	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS(NetworkConfig)
	...
	namespace {
		struct NetworkConfigTransactionTraits {
			static auto GenerateEntityWithAttachments(uint16_t networkConfigSize, uint16_t supportedEntityVersionsSize) {
			...
			static constexpr size_t GetAttachment1Size(uint16_t networkConfigSize) {
				return networkConfigSize;
			}
	...
	DEFINE_DUAL_ATTACHMENT_POINTER_TESTS(TEST_CLASS, NetworkConfigTransactionTraits)
