...
#define TEST_CLASS CatapultConfigSignerValidatorTests

	DEFINE_COMMON_VALIDATOR_TESTS(CatapultConfigSigner,)

	namespace {
		void AssertValidationResult(const Key& signer, const Key& networkKey, const ValidationResult& expectedResult) {
			// Arrange:
			auto notification = model::CatapultConfigSignerNotification<1>(signer);
			...
			auto pValidator = CreateCatapultConfigSignerValidator();
	...
	TEST(TEST_CLASS, FailureWhenSignerAndNetworkPublicKeyDontMatch) {
		// Assert:
		AssertValidationResult(test::GenerateKeyPair().publicKey(), test::GenerateKeyPair().publicKey(), Failure_CatapultConfig_Invalid_Signer);
	}
}}
