...
#define TEST_CLASS CatapultConfigTransactionPluginTests

	namespace {
		constexpr auto Transaction_Version = MakeVersion(NetworkIdentifier::Mijin_Test, 1);

		DEFINE_TRANSACTION_PLUGIN_TEST_TRAITS(CatapultConfig, 1, 1,)

		template<typename TTraits>
		auto CreateTransactionWithAttachments(uint16_t blockChainConfigSize, uint16_t supportedEntityVersionsSize) {
	...
	DEFINE_BASIC_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS(TEST_CLASS,,, Entity_Type_Catapult_Config)
	...
	namespace {
		...
		void AssertNumNotifications(
			...
			EXPECT_EQ(CatapultConfig_Signer_v1_Notification, sub.notificationTypes()[0]);
			EXPECT_EQ(CatapultConfig_Catapult_Config_v1_Notification, sub.notificationTypes()[1]);
		}
	}
	...
	// region publish - catapult config notification

	PLUGIN_TEST(CanPublishCatapultConfigNotification) {
		// Arrange:
		mocks::MockTypedNotificationSubscriber<CatapultConfigNotification<1>> sub;
		auto pPlugin = TTraits::CreatePlugin();
	...
	// region publish - catapult config signer notification

	PLUGIN_TEST(CanPublishCatapultConfigSignerNotification) {
		// Arrange:
		mocks::MockTypedNotificationSubscriber<CatapultConfigSignerNotification<1>> sub;
		auto pPlugin = TTraits::CreatePlugin();
