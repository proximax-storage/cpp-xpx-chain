...
#define TEST_CLASS NetworkConfigAbsoluteHeightTransactionPluginTests
	...
	namespace {
		...
		void AssertNumNotifications(
			...
			EXPECT_EQ(NetworkConfig_Network_Config_v2_Notification, sub.notificationTypes()[1]);
		}
	}
	...
	PLUGIN_TEST(CanPublishNetworkConfigNotification) {
		// Arrange:
		mocks::MockTypedNotificationSubscriber<NetworkConfigNotification<2>> sub;
		auto pPlugin = TTraits::CreatePlugin();
		...
		EXPECT_EQ(100, notification.ApplyHeight);
