...
namespace catapult { namespace validators {
		...
		DECLARE_STATEFUL_VALIDATOR(NetworkConfig, Notification)(const plugins::PluginManager& pluginManager) {
			return MAKE_STATEFUL_VALIDATOR(NetworkConfig, ([&pluginManager](const Notification& notification, const StatefulValidatorContext& context) {
			  ...
			  for (const auto& pair : networkConfig.Plugins) {
				  auto result = pValidator->validate(
						  model::PluginConfigNotification<1>{pair.first, pair.second},
						  // We want to verify a new plugins with old config
						  validators::StatelessValidatorContext(context.Config)
				  );
