...
#define TEST_CLASS AddHarvesterTransactionPluginTests

	namespace {
		DEFINE_TRANSACTION_PLUGIN_TEST_TRAITS(AddHarvester, 1, 1,)
	...
	DEFINE_BASIC_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS(TEST_CLASS,,, Entity_Type_AddHarvester)
	...
	PLUGIN_TEST(CanPublishCorrectNumberOfNotifications) {
		...
		EXPECT_EQ(Committee_AddHarvester_v1_Notification, sub.notificationTypes()[0]);
	}
	...
	PLUGIN_TEST(CanPublishAddHarvesterNotification) {
		// Arrange:
		mocks::MockTypedNotificationSubscriber<AddHarvesterNotification<1>> sub;
		auto pPlugin = TTraits::CreatePlugin();
