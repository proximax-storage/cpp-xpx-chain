...
#define TEST_CLASS WeightedVotingCommitteeManagerTests
	...
	TEST(TEST_CLASS, WeightedVotingCommitteeManager_ReturnsEmptyCommitteeWhenNoCandidates) {
		RunTest(std::make_shared<cache::CommitteeAccountCollector>(), Committee());
	}

	TEST(TEST_CLASS, WeightedVotingCommitteeManager_ReturnsAllCandidates) {
		Committee expectedCommittee;
		auto pAccountCollector = std::make_shared<cache::CommitteeAccountCollector>();

		// The number of accounts is less than the committee number, hence all accounts are selected to the committee.
		// All accounts have the same greed, so the block proposer is the account with the minimum hit.
		// Account hit is the first 8 bytes of the account key because the mock hasher doesn't do any transformation,
		// but just copies the key to hash.
		auto minHit = std::numeric_limits<uint64_t>::max();
		for (uint8_t i = 1u; i <= Committee_Number / 2; ++i) {
	...
	TEST(TEST_CLASS, WeightedVotingCommitteeManager_SelectsCommittee) {
		...
		// First Committee_Number accounts have the same effective balance and increasing hits, hence account rates are
		...
		pAccountCollector->addAccount(state::CommitteeEntry(
			...
			test::CreateAccountData(Height(), Importance(Committee_Number), true, test::Initial_Activity, 0.8)));
		for (uint8_t i = 2u; i <= Committee_Number; ++i) {
			auto key = Key{ { i } };
			expectedCommittee.Cosigners.insert(key);
			pAccountCollector->addAccount(state::CommitteeEntry(
				key, test::CreateAccountData(Height(), Importance(Committee_Number), true, test::Initial_Activity, 0.8 / i)));
		}
		...
		for (uint8_t i = Committee_Number + 1u; i <= Account_Number; ++i)
