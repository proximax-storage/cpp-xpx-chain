...
namespace catapult { namespace state {
	...
	namespace {
		void SavePatriciaTreeCommitteeEntry(const CommitteeEntry& entry, io::OutputStream& output) {
			// write version
		...
		CommitteeEntry LoadPatriciaTreeCommitteeEntry(io::InputStream& input) {
			// read version
			...
			return state::CommitteeEntry(key, owner, lastSigningBlockHeight, effectiveBalance, canHarvest, 0.0, 0.0, disabledHeight, version);
		}
	}

	void CommitteeEntrySerializer::Save(const CommitteeEntry& entry, io::OutputStream& output) {
		SavePatriciaTreeCommitteeEntry(entry, output);
	...
	CommitteeEntry CommitteeEntrySerializer::Load(io::InputStream& input) {
		auto entry = LoadPatriciaTreeCommitteeEntry(input);
		entry.setActivity(io::ReadDouble(input));
		entry.setGreed(io::ReadDouble(input));
	...
	CommitteeEntry CommitteeEntryPatriciaTreeSerializer::Load(io::InputStream& input) {
		return LoadPatriciaTreeCommitteeEntry(input);
	}
}}
