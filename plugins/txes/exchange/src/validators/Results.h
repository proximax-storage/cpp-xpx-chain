...
#define DEFINE_CATAPULT_EXCHANGE_RESULT(DESCRIPTION, CODE) DEFINE_VALIDATION_RESULT(Failure, Exchange, DESCRIPTION, CODE, None)
	...
	/// Validation failed because offer deadline is not in future.
	DEFINE_CATAPULT_EXCHANGE_RESULT(Past_Offer_Deadline, 5);

	/// Validation failed because there is no offers.
	DEFINE_CATAPULT_EXCHANGE_RESULT(No_Offers, 6);

	/// Validation failed because mosaic for exchange is invalid.
	DEFINE_CATAPULT_EXCHANGE_RESULT(Invalid_Mosaic, 7);

	/// Validation failed because offer has already been removed.
	DEFINE_CATAPULT_EXCHANGE_RESULT(Offer_Already_Removed, 8);
	...
	DEFINE_CATAPULT_EXCHANGE_RESULT(Unit_Not_Found_In_Offer, 10);

	/// Validation failed because there is not enough units in offer.
	DEFINE_CATAPULT_EXCHANGE_RESULT(Not_Enough_Units_In_Offer, 11);

	/// Validation failed because price is invalid in offer.
	DEFINE_CATAPULT_EXCHANGE_RESULT(Invalid_Price, 12);
