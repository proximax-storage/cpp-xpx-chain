...
namespace catapult { namespace observers {
	...
	DEFINE_OBSERVER(ExchangeV1, model::ExchangeNotification<1>, [](const auto& notification, const ObserverContext& context) {
	...
	DEFINE_OBSERVER(ExchangeV2, model::ExchangeNotification<2>, [](const auto& notification, const ObserverContext& context) {
		auto& cache = context.Cache.sub<cache::ExchangeCache>();
		auto pMatchedOffer = notification.OffersPtr;
		for (uint8_t i = 0; i < notification.OfferCount; ++i, ++pMatchedOffer) {
			auto mosaicId = context.Resolvers.resolve(pMatchedOffer->Mosaic.MosaicId);
