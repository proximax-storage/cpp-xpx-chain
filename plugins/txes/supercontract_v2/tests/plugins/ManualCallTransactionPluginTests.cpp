...
namespace catapult { namespace plugins {
	...
	#define TEST_CLASS ManualCallTransactionPluginTests

	namespace {
		DEFINE_TRANSACTION_PLUGIN_WITH_CONFIG_TEST_TRAITS(ManualCall, config::ImmutableConfiguration, 1, 1, )
		...
		auto CreateTransaction() {
			return test::CreateManualCallTransaction<typename TTraits::TransactionType>();
		}
	...
	DEFINE_BASIC_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS(TEST_CLASS, , , Entity_Type_ManualCallTransaction, CreateConfiguration())
	...
	PLUGIN_TEST(CanPublishManualCallNotification) {
		...
		auto pTransaction = CreateTransaction<TTraits>();
		const std::string fileName(reinterpret_cast<const char*>(pTransaction->FileNamePtr()), pTransaction->FileNameSize);
		const std::string functionName(reinterpret_cast<const char*>(pTransaction->FunctionNamePtr()), pTransaction->FunctionNameSize);
		const std::string actualArguments(reinterpret_cast<const char*>(pTransaction->ActualArgumentsPtr()), pTransaction->ActualArgumentsSize);
		...
		EXPECT_EQ(fileName, notification.FileName);
		EXPECT_EQ(functionName, notification.FunctionName);
		EXPECT_EQ(actualArguments, notification.ActualArguments);
	...
	PLUGIN_TEST(CanPublishCreditMosaicNotification) {
		...
		auto pPlugin = TTraits::CreatePlugin(config);
		auto pTransaction = CreateTransaction<TTraits>();
		auto contractKey = Key(pTransaction->ContractKey);
		Hash256 paymentHash;
