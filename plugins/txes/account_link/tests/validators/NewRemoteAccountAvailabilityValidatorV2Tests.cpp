...
#define TEST_CLASS NewRemoteAccountAvailabilityV2ValidatorTests

	DEFINE_COMMON_VALIDATOR_TESTS(NewRemoteAccountAvailabilityV2,)
	...
	namespace {
		...
		cache::CatapultCache CreateCache(const Address& address, state::AccountType accountType) {
			auto cache = test::CoreSystemCacheFactory::Create();
			auto cacheDelta = cache.createDelta();
			auto& accountStateCacheDelta = cacheDelta.sub<cache::AccountStateCache>();
			accountStateCacheDelta.addAccount(address, Height(1));
			accountStateCacheDelta.find(address).get().AccountType = accountType;
			cache.commit(Height(1));
			return cache;
		}
		...
		void AssertValidation(ValidationResult expectedResult, const cache::CatapultCache& cache, const Key& publicKey) {
			// Arrange:
			auto pValidator = CreateNewRemoteAccountAvailabilityV2Validator();
			auto notification = model::NewRemoteAccountNotification<2>(publicKey);
		...
		void AssertFailsWhenAccountExistsWithTypeAddress(state::AccountType accountType) {
			// Arrange:
			auto publicKey = test::GenerateRandomByteArray<Key>();
			auto address = model::PublicKeyToAddress(publicKey, model::NetworkIdentifier::Zero);
			auto cache = CreateCache(address, accountType);
	...
	TEST(TEST_CLASS, FailsWhenAccountExistsAddressAndHasTypeMain) {
		// Assert:
		AssertFailsWhenAccountExistsWithTypeAddress(state::AccountType::Main);
	}

	TEST(TEST_CLASS, FailsWhenAccountExistsAddressAndHasTypeUnlinked) {
		// Assert:
		AssertFailsWhenAccountExistsWithTypeAddress(state::AccountType::Unlinked);
	}

	TEST(TEST_CLASS, FailsWhenAccountExistsAndHasTypeRemote) {
		// Assert:
		AssertFailsWhenAccountExistsWithTypeAddress(state::AccountType::Remote);
	}
