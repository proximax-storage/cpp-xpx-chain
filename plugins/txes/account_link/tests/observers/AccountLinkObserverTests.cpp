...
#define ACCOUNT_LINK_OBSERVER_TEST(TEST_NAME) \
	...
	ACCOUNT_LINK_OBSERVER_TEST(ObserverCreatesLink) {
		...
		RunTwoAccountTest(context.cache().sub<cache::AccountStateCache>(), [&context](
			...
			EXPECT_EQ(remoteAccountKey, mainAccountState.SupplementalPublicKeys.linked().get());

			EXPECT_EQ(state::AccountType::Remote, remoteAccountState.AccountType);
			EXPECT_EQ(mainAccountKey, remoteAccountState.SupplementalPublicKeys.linked().get());
		});
	}
	...
	ACCOUNT_LINK_OBSERVER_TEST(ObserverRemovesLink) {
			...
			mainAccountState.SupplementalPublicKeys.linked().set(remoteAccountKey);
			mainAccountState.AccountType = state::AccountType::Main;

		  	remoteAccountState.SupplementalPublicKeys.linked().unset();
			remoteAccountState.SupplementalPublicKeys.linked().set(mainAccountKey);
			...
			EXPECT_EQ(Key(), mainAccountState.SupplementalPublicKeys.linked().get());

			EXPECT_EQ(state::AccountType::Remote_Unlinked, remoteAccountState.AccountType);
			EXPECT_EQ(Key(), remoteAccountState.SupplementalPublicKeys.linked().get());
		});
	}
}}
