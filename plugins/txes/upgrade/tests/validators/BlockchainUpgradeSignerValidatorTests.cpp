...
#define TEST_CLASS BlockchainUpgradeSignerValidatorTests

	DEFINE_COMMON_VALIDATOR_TESTS(BlockchainUpgradeSigner,)

	namespace {
		void AssertValidationResult(const Key& signer, const Key& networkKey, const ValidationResult& expectedResult) {
			// Arrange:
			auto notification = model::BlockchainUpgradeSignerNotification<1>(signer);
			...
			auto pValidator = CreateBlockchainUpgradeSignerValidator();
	...
	TEST(TEST_CLASS, FailureWhenSignerAndNetworkPublicKeyDontMatch) {
		// Assert:
		AssertValidationResult(test::GenerateKeyPair().publicKey(), test::GenerateKeyPair().publicKey(), Failure_BlockchainUpgrade_Invalid_Signer);
	}
}}
