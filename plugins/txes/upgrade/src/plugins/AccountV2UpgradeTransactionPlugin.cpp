...
namespace catapult { namespace plugins {
	...
	namespace {
		...
		void Publish(const TTransaction& transaction, const Height&, NotificationSubscriber& sub) {
			...
			default:
				CATAPULT_LOG(debug) << "invalid version of AccountV2UpgradeTransaction: " << transaction.EntityVersion();
			}
		}
	}

	DEFINE_TRANSACTION_PLUGIN_FACTORY(AccountV2Upgrade, Default, Publish)
}}
