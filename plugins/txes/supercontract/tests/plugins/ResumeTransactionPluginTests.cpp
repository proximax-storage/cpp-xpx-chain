...
#define TEST_CLASS ResumeTransactionPluginTests

	// region TransactionPlugin

	namespace {
		DEFINE_TRANSACTION_PLUGIN_TEST_TRAITS(Resume, 1, 1,)

		static const auto Generation_Hash = test::GenerateRandomByteArray<GenerationHash>();

		template<typename TTraits>
		auto CreateTransaction() {
			return test::CreateResumeTransaction<typename TTraits::TransactionType>();
		}
	}

	DEFINE_BASIC_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS(TEST_CLASS,,, Entity_Type_Resume)
	...
	PLUGIN_TEST(CanPublishCorrectNumberOfNotifications) {
		...
		EXPECT_EQ(SuperContract_Resume_v1_Notification, sub.notificationTypes()[i++]);
	}
	...
	// region resume notification

	PLUGIN_TEST(CanPublishResumeNotification) {
		// Arrange:
		mocks::MockTypedNotificationSubscriber<ResumeNotification<1>> sub;
		auto pPlugin = TTraits::CreatePlugin();
		auto pTransaction = CreateTransaction<TTraits>();
