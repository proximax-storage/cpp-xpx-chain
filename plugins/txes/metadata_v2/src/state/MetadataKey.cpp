...
namespace catapult { namespace state {
	...
	const Key& MetadataKey::targetKey() const {
		return m_partialKey.TargetKey;
	}
	...
	Hash256 MetadataKey::generateUniqueKey() const {
		...
		for (const auto* pKey : { &m_partialKey.SourceAddress })
			builder.update(*pKey);

		for (const auto* pKey : { &m_partialKey.TargetKey })
			builder.update(*pKey);
