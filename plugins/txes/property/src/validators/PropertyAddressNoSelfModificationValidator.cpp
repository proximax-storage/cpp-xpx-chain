...
namespace catapult { namespace validators {
	...
	DECLARE_STATEFUL_VALIDATOR(PropertyAddressNoSelfModification, Notification)(const std::shared_ptr<config::BlockchainConfigurationHolder>& pConfigHolder) {
		return MAKE_STATEFUL_VALIDATOR(PropertyAddressNoSelfModification, [pConfigHolder](const auto& notification, const auto& context) {
			...
			auto address = model::PublicKeyToAddress(notification.Key, config.Info.Identifier);
