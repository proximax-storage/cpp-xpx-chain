...
namespace catapult { namespace model {
	...
	using TransactionType = DriveFileSystemTransaction;

#define TEST_CLASS DriveFileSystemTransactionTests
	...
	namespace {
		...
		void AssertTransactionHasExpectedProperties() {
			// Assert:
			EXPECT_EQ(Entity_Type_DriveFileSystem, static_cast<EntityType>(T::Entity_Type));
			EXPECT_EQ(1u, static_cast<VersionType>(T::Current_Version));
		}
	}

	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS(DriveFileSystem)
	...
	namespace {
		struct DriveFileSystemTransactionTraits {
	...
	DEFINE_DUAL_ATTACHMENT_POINTER_TESTS(TEST_CLASS, DriveFileSystemTransactionTraits)
