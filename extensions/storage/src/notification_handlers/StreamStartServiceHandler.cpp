...
namespace catapult { namespace notification_handlers {
	...
	using Notification = model::StreamStartServiceNotification<1>;

	DECLARE_HANDLER(StreamStartService, Notification)(const std::weak_ptr<storage::ReplicatorService>& pReplicatorServiceWeak) {
		return MAKE_HANDLER(StreamStartService, [pReplicatorServiceWeak](const Notification& notification, const HandlerContext& context) {
