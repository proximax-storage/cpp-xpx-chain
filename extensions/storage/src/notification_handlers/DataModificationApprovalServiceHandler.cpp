...
namespace catapult { namespace notification_handlers {
    ...
    using Notification = model::DataModificationApprovalServiceNotification<1>;

    DECLARE_HANDLER(DataModificationApprovalService, Notification)(const std::weak_ptr<storage::ReplicatorService>& pReplicatorServiceWeak) {
        return MAKE_HANDLER(DataModificationApprovalService, [pReplicatorServiceWeak](const Notification& notification, const HandlerContext& context) {
