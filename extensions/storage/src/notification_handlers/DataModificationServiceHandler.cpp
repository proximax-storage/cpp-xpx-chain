...
namespace catapult { namespace notification_handlers {
	...
	using Notification = model::DataModificationServiceNotification<1>;

	DECLARE_HANDLER(DataModificationService, Notification)(const std::weak_ptr<storage::ReplicatorService>& pReplicatorServiceWeak) {
		return MAKE_HANDLER(DataModificationService, [pReplicatorServiceWeak](const Notification& notification, const HandlerContext& context) {
