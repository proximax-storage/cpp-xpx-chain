...
#define TEST_CLASS RollbackInfoTests
    ...
    namespace {
		...
		auto CreateDefaultRollbackInfo() {
			auto config = const_cast<model::NetworkConfiguration&>(Default_Service_State.state().config().Network);
			config.BlockGenerationTargetTime = utils::TimeSpan::FromSeconds(15);
			config.MaxRollbackBlocks = 40;
			return RollbackInfo(test::CreateTimeSupplierFromMilliseconds({ 1 }), Default_Service_State.state());
		}
	}
	...
	namespace {
		...
		void AssertOperationPrunesRecentStats(const consumer<RollbackInfoModifier&>& actInfo,
			...
			RollbackInfo info(test::CreateTimeSupplierFromMilliseconds(rawTimestamps), serviceState.state());
