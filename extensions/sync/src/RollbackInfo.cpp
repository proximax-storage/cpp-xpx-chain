...
// region RollbackInfoView
	...
	RollbackInfo::RollbackInfo(chain::TimeSupplier timeSupplier, extensions::ServiceState& state)
			: m_state(state)
	{
		auto rollbackDurationFull = CalculateFullRollbackDuration(m_state.config().Network);
		auto recentStatsTimeSpan = utils::TimeSpan::FromMilliseconds(rollbackDurationFull.millis() / 2);

		 m_pRollbackInfoState = std::make_unique<RollbackInfoState>(timeSupplier, recentStatsTimeSpan);
	} 
	...
	RollbackInfoModifier RollbackInfo::modifier() {
		...
		return RollbackInfoModifier(m_committed, m_ignored, *m_pRollbackInfoState, std::move(writeLock));
	}
