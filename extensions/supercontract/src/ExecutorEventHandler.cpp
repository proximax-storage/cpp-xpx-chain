...
namespace catapult { namespace contract {
	...
	void ExecutorEventHandler::endBatchTransactionIsReady(
		...
		//Hash256(transactionInfo.m_contractKey.array()); 		std::vector<Key> publicKeys; 		std::vector<Signature>
		//signatures; 		std::vector<model::RawProofOfExecution> proofs; 		std::vector<model::ExtendedCallDigest>
		//callDigests; 		std::vector<model::CallPayment> callPayments;
		//
	...
	void ExecutorEventHandler::endBatchTransactionIsReady(
		...
		//Hash256(transactionInfo.m_contractKey.array()); 		std::vector<Key> publicKeys; 		std::vector<Signature>
		//signatures; 		std::vector<model::RawProofOfExecution> proofs; 		std::vector<model::ShortCallDigest> callDigests;
