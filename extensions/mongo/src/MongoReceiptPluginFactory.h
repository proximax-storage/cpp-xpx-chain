...
#define DEFINE_MONGO_RECEIPT_PLUGIN_FACTORY(NAME, STREAM) \
	...
	/// Creates a mongo signer balance receipt plugin around \a type.
	std::unique_ptr<MongoReceiptPlugin> CreateSignerBalanceReceiptMongoPlugin(model::ReceiptType type);

	/// Creates a mongo drive state receipt plugin around \a type.
	std::unique_ptr<MongoReceiptPlugin> CreateDriveStateReceiptMongoPlugin(model::ReceiptType type);

	/// Creates a mongo global state change receipt plugin around \a type.
	std::unique_ptr<MongoReceiptPlugin> CreateGlobalStateChangeReceiptMongoPlugin(model::ReceiptType type);
