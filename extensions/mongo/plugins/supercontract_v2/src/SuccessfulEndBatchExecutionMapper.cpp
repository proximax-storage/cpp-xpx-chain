...
namespace catapult { namespace mongo { namespace plugins {
	...
	void StreamSuccessfulEndBatchExecutionTransaction(bson_stream::document& builder, const TTransaction& transaction) {
		builder << "contractKey" << ToBinary(transaction.ContractKey) << "batchId"
				...
				<< ToInt64(transaction.AutomaticExecutionsNextBlockToCheck) << "storageHash"
				<< ToBinary(transaction.StorageHash) << "usedSizeBytes"
				<< static_cast<int64_t>(transaction.UsedSizeBytes) << "metaFilesSizeBytes"
				<< static_cast<int64_t>(transaction.MetaFilesSizeBytes) << "proofOfExecutionVerificationInformation"
				<< ToBinary(
						   transaction.ProofOfExecutionVerificationInformation.data(),
