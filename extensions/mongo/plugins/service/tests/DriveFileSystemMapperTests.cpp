...
#include "src/DriveFileSystemMapper.h"
...
#define TEST_CLASS DriveFileSystemMapperTests

	namespace {
		DEFINE_MONGO_TRANSACTION_PLUGIN_TEST_TRAITS_NO_ADAPT(DriveFileSystem,)
		...
		void AssertCanMapDriveFileSystemTransaction(size_t numAddActions, size_t numRemoveActions) {
			// Arrange:
			auto pTransaction = test::CreateDriveFileSystemTransaction<typename TTraits::TransactionType>(numAddActions, numRemoveActions);
	...
	DEFINE_BASIC_MONGO_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS(TEST_CLASS,,, model::Entity_Type_DriveFileSystem)

	PLUGIN_TEST(CanMapDriveFileSystemTransactionWithoutOffers) {
		// Assert:
		AssertCanMapDriveFileSystemTransaction<TTraits>(0, 0);
	}

	PLUGIN_TEST(CanMapDriveFileSystemTransactionWithOffers) {
		// Assert:
		AssertCanMapDriveFileSystemTransaction<TTraits>(3, 4);
	}
}}}
