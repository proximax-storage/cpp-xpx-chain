...
namespace catapult { namespace test {
	...
	void AssertEqualCatapultConfigData(const state::CatapultConfigEntry& entry, const bsoncxx::document::view& dbConfigEntry) {
		EXPECT_EQ(entry.height(), mongo::mappers::GetValue64<Height>(dbConfigEntry["height"]));
		EXPECT_EQ(entry.blockChainConfig(), dbConfigEntry["blockChainConfig"].get_utf8().value.to_string());
