...
#include "mongo/src/mappers/BlockchainStateStatementMapper.h"
...
#define TEST_CLASS TransactionStatementMapperTests
	...
	namespace {
		void AssertCanMapBlockchainStateStatement(size_t numReceipts) {
			// Arrange:
			model::ReceiptSource source(123, 234);
			model::BlockchainStateStatement statement(source);
			...
			// Assert:
			test::AssertEqualBlockchainStateStatement(statement, Height(567), document.view(), 3, 0);
		}
	}

	TEST(TEST_CLASS, CanMapBlockchainStateStatement_NoReceipts) {
		// Assert:
		AssertCanMapBlockchainStateStatement(0);
	}

	TEST(TEST_CLASS, CanMapBlockchainStateStatement_SingleReceipt) {
		// Assert:
		AssertCanMapBlockchainStateStatement(1);
	}

	TEST(TEST_CLASS, CanMapBlockchainStateStatement_MultipleReceipts) {
		// Assert:
		AssertCanMapBlockchainStateStatement(5);
	}
}}}
