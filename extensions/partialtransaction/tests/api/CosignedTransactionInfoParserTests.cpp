...
#define ASSERT_EQ_COSIGNATURES(EXPECTED, ACTUAL) \
	...
	namespace {
		...
		public:
			explicit HashGenerator(uint8_t numCosignatures)
					...
					, m_cosignatures(test::GenerateRandomDataVector<model::Cosignature<CoSignatureVersionAlias::Raw>>(numCosignatures))
			{}
		...
		private:
			...
			Hash256 m_hash;
			std::vector<model::Cosignature<CoSignatureVersionAlias::Raw>> m_cosignatures;
		};
	...
	TEST(TEST_CLASS, FailureWhenCosignatureCannotBeExtracted) {
		...
		AssertParseFailure([](auto& builder) {
			...
			AppendValues(builder, test::GenerateRandomDataVector<model::Cosignature<CoSignatureVersionAlias::Raw>>(1));
		});
	}
