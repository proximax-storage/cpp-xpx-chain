...
#define TEST_CLASS ValidateHarvestingConfigurationTests
	...
	namespace {
		...
		HarvestingConfiguration CreateHarvestingConfiguration(
			...
			harvestingConfig.HarvestKey = harvesterSigningPrivateKey;
			harvestingConfig.HarvesterVrfPrivateKey = harvesterVrfPrivateKey;
			harvestingConfig.IsAutoHarvestingEnabled = enableAutoHarvesting;
	...
	// region key validity
		...
		TEST(TEST_CLASS, ValidationFailsWhenAnyHarvesterPrivateKeyIsInvalid) {
			...
			AssertInvalidHarvestingConfiguration(CreateHarvestingConfiguration(Valid_Key, Valid_Key, true, Invalid_Key));
		}
