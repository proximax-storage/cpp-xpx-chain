...
#define TEST_CLASS HarvestingServiceTests
	...
	namespace {
		...
		public:
			...
			explicit TestContext(cache::CatapultCache&& currentCache, cache::CatapultCache&& previousCache)
					: BaseType(std::move(currentCache), std::move(previousCache))
	...
	TEST(TEST_CLASS, HarvestingTaskDoesNotPruneEligibleAccount) {
		...
		TestContext context(CreateCacheWithAccount(height, keyPair.publicKey()), CreateCacheWithAccount(height, keyPair.publicKey()));
